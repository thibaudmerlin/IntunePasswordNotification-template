...
if ($user) {
    ...
    $userId
    if ($userId) {
        $dayQuery = "users/{0}?`$select=lastPasswordChangeDateTime" -f $userId
        $dayLeft = (Get-JsonFromGraph -token $token -strQuery $dayQuery -ver v1.0).lastPasswordChangeDateTime
        [datetime]$lastpasswordChange = $dayLeft -replace "Z","" -replace "T"," "
        $days = $parameters.expirationperiod
        $PasswordExpirationDate = ($lastpasswordChange).AddDays($days)
        $StartDate  = (Get-Date).ToString("yyyy-MM-dd HH:mm:ss")
        $TimeSpan = New-Timespan -Start $StartDate -End $PasswordExpirationDate
    }
    $expToMap = $parameters.expirationPeriod
    $notifToMap = $parameters.notificationPeriod
    ...
    $result = [PSCustomObject]@{
        ...
        expirationPeriod            = $expToMap | Where-Object { $_ }
        notificationPeriod          = $notifToMap | Where-Object { $_ }
